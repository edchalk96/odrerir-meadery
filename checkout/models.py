...
class OrderLineItem(models.Model):
    ...
    product = models.ForeignKey(Product, null=False,
                                blank=False, on_delete=models.CASCADE)
    product_volume = models.CharField(max_length=10, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.product_volume:
            unit_price = self.product.calculate_price_for_volume(
                self.product_volume)
        else:
        ...
        if not self.pk:
            volume_in_liters = self.product.get_volume_in_liters(
                self.product_volume)
    ...
    def __str__(self):
        volume_str = f' ({self.product_volume})' if self.product_volume else ''
        return f'SKU {self.product.sku or self.product.name}{
            volume_str} on order {self.order.order_number}'
